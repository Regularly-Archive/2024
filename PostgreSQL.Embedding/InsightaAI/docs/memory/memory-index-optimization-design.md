...
手动 `search_memory` 保持宽召回，供 Agent 主动探索；只有构建 `ActiveMemorySnapshot` 时才应用严格筛选。当前的初始规则为：

- 输入至少应产生 3 个非空白 trigram 查询片段；更短或过于宽泛的输入不会自动注入 OnDemand 记忆。
- 候选至少命中 2 个查询片段，且命中片段数至少覆盖查询片段的 50%。
- “我是谁”一类带有明确 `User` 类型意图的查询保留类型筛选的既有兜底行为。
- `20` 只作为候选检索与最终快照的安全上限，不是每轮应注入的目标数量；筛选后可以没有任何 OnDemand 条目。

这些是保守的初始值，不是基于大规模数据得出的固定参数。`MemoryManager` 在本地 Debug 日志记录候选 ID、类型、命中片段数、覆盖率和入选/淘汰原因；不记录用户输入或记忆正文。CLI 仅为该分类开启 Debug 文件日志，其余分类维持 Information。后续根据真实会话校准。
