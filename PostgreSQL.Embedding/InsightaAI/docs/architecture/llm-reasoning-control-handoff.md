...
>
> 验证状态：`InsightaAI.Agent.Cli.Tests` 11/11、`InsightaAI.LLM.Tests` 124/124 通过；`dotnet build InsightaAI.sln --no-restore` 成功。Anthropic `budget_tokens < max_tokens` 仍为 TODO #21 的未决 P1；GLM 在线验证继续因额度不足而搁置。
