...
- **Top 5 tools by success/failure rate**：按相应比率选出前五名，并分别显示 `Success rate`、`Failure rate`。分子和分母都限定 `gen_ai_tool_is_allowed=true`，因此权限拒绝不被计为执行成功或工具故障。
- **Top 5 most frequently activated skills**：显示按 `insighta_skill_name` 聚合的前五个成功激活 Skill 及其激活次数。只在 `activate_skill` 被允许且成功时计数，不把失败、拒绝或未找到的 Skill 计入。Top N 排行不使用会按当前最大值填满的 Bar gauge，避免少量候选时放大视觉信号。
