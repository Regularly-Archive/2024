...
Open Grafana at `http://localhost:3000` (default credentials: `admin` / `admin`). It provisions a Prometheus data source, a Jaeger trace data source, and the **InsightaAI Overview** plus **InsightaAI Tools** dashboards. Jaeger remains directly available at `http://localhost:16686`; Prometheus is at `http://localhost:9090`.
