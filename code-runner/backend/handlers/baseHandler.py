...
class BaseHandler(ABC):
    ...
    def cleanup(self):
        self.client.cleanup_container(self.container, self.ctx.project_dir, True)
    ...
    def execute_stage(self, stage: str) -> StageResult:
        ...
        exit_code = self.client.run_command(self.container, wrapped_cmd, self.ctx.runtime_info.user)
        duration = time.time() - start_time
        return StageResult(
            name=stage, 
            command=cmd, 
            exit_code=exit_code,
            stdout=self._read_output(self.ctx.project_dir, stage, 'stdout.txt'),
            stderr=self._read_output(self.ctx.project_dir, stage, 'stderr.txt'),
            duration=duration
        )
